---
name: pre-loop
description: Run BEFORE a loop (Codex Goal /goal or an Automation) to design its full operating context. Reads your repo to infer architecture/conventions/constraints, asks you only for the gaps, solidifies the architecture foundations, builds and guards a real verifier (anti-gaming + pre-flight run), sets up safe/reversible execution and loop-health signals, writes loop-brief.md, and hands back a ready-to-run /goal command. Use when the user wants to set up a goal, automate a recurring task, says "loop this" / "keep going until…", or asks how to loop something.
---
...
1. **Read the repo first.** Infer from `AGENTS.md`/`CLAUDE.md`, README,
...
2. **Then run as a short wizard.** Present what you inferred, then confirm and fill
   the gaps with **concise terminal questions** (one at a time — fit, scope,
   run-on-a-branch?, approval boundary, which verifier). Ask only for what you
   couldn't infer — don't interrogate.
...
> If the repo has no durable context file (`AGENTS.md` / `CLAUDE.md`) or it's thin,
...
2. **Durable context** — note any `AGENTS.md` / `CLAUDE.md` you wrote.
3. **The seed (not just the end-state)** — hand back the full kickoff: the `/goal` end-state **plus** the first instruction (which files to read, the plan, the brief) and **golden examples** to mirror ("follow `cache.py`'s structure; here's the PR that did the last one right").
   - Completion → Codex **Goal mode** `/goal "<verifiable end state, pointed at the verifier stack>"`
   - Scheduled / recurring → a Codex **Automation** (in the Codex app): use the loop-brief as the prompt, set the schedule/trigger, route results to a review queue.
...
## Rules
- Infer before you ask; never interrogate.
